...
Two checks and two separate suites:
...
| `npm run boundaries` | boundary check (dependency-cruiser) | Resolves the import graph of `src/` and refuses the crossings of [ARC-14.2](docs/REQUIREMENTS.md) — today, no `excalibur` under `engine/`. Type-only imports count. It runs as part of `npm run test:unit` and of `npm run build`, so a crossing fails the build (ARC-14.3). |
| `npm run test:unit` | headless (Vitest) | Lints, typechecks and boundary-checks the project, then runs the `*.spec.ts` files under `src/` and `tests-headless/` in Node. No browser, no server. |
...
Every pull request runs `npm ci && npm run test:unit` on GitHub Actions
([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), on the Node version declared in `.nvmrc`.
CI runs the same scripts a developer runs — it adds none of its own, so the boundary check arrived
there by being part of `test:unit`.

The **push** trigger currently names `master_disabled` rather than `master`, so pushes to `master`
run nothing. That is deliberate and reversible in one line.
